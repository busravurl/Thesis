...
import {SafeAreaView, Text,TextInput, FlatList, View, Dimensions, StatusBar} from 'react-native';
...
const Home = () => {
    ...
    const sendContent = async (event) =>{
        ...
        try {
            const response = await axios.post('http://192.168.1.106:45455/api/cebimdekiBahcivan/BenimBahcemeBitkiEkleme', {
                bitkiAd,
                notlar,
    ...
    const getList = async (event) => {
        try {
        const response = await axios.get('http://192.168.1.106:45455/api/cebimdekiBahcivan/BenimBahcem', {
            BitkiAd,
            Notlar,
